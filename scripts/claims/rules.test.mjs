...
// `controls`. Run with `npm run guard:claims:test`; `prebuild` runs it ahead of
// the scan itself, so a bad regex edit fails the deploy rather than going quiet.
//
// `selfTest` is accepted as an alias for `controls`: banned-claims.json is a
// GENERATED artifact (sync-proof-table.mjs, from the ARY-23 §0 claims-guard
// block), so the key name is whatever the source doc used on the day a rule was
// authored — file-link-control-framing says `selfTest`. Renaming it here would
// be reverted by the next `claims:sync`, and treating it as absent would report
// a rule that HAS controls as uncovered, which is precisely the silent gap this
...
for (const rule of rules) {
  const controls = rule.controls ?? rule.selfTest
  if (!controls) {
    uncovered.push(rule.id)
    continue
  }
  const { mustFire = [], mustStayGreen = [] } = controls
